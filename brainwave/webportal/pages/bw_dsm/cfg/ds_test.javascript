...
function check() {
	try {
		var applicationcode = dataset.applicationcode.get();
		var sourcefile = dataset.filepath.get();
		var checker = new com.brainwave.utils.CSVFormatChecker();
	
		// get general configuration
		var params = new java.util.HashMap();
	  	params.put("code", applicationcode );
	  	var res = connector.executeView(config.__TIMESLOT_UID, "bw_getdatasourceconfig" , params);
	  	if(res == null || res.length == 0) {
			dataset.errors.add("Error, no configuration found. Most probably because no configuration are associated with this application");
			return;
	  	}
		...
		// get columns definition
	  	var res = connector.executeView(config.__TIMESLOT_UID, "bw_getdatasourcecolumns" , params);
	  	if(res == null || res.length == 0) {
			dataset.errors.add("Error, no columns defined. You MUST declare at least ONE column in the file configuration");
			return;
	  	}
		...
		// check the file size
		if(checksize) {
			var previoussize = dataset.isEmpty('previoussize')?0:dataset.previoussize.get();	
			if(previoussize>0 && checksizevariation>0) {
				...
				if(currentsize<low || currentsize>high) {
					dataset.errors.add("Error, the current file size is inconsistent as compared with the previous one. Current file size is "+currentsize+" previous file size is "+previoussize+" variation threshold is set to +/- "+checksizevariation+"%");
					return;
				}
			}
		}
		...
		if(result!=null) {
			for(var i=0;i<result.length;i++)
				dataset.errors.add(result[i]);
		}

		var totalnbrows = checker.getTotalNbOfRecords();
		dataset.totalnbrows.set(totalnbrows);
	}catch(e) {
		dataset.errors.add("Unkown system error while checking the file");
...
function transcode(  ) {
	...
	var /*String*/ code = dataset.applicationcode.get();
	var /*String*/ sourcefilepath = dataset.sourcefile.get();
	var /*String*/  destfilepath = dataset.destinationfile.get();
	...
	var res = connector.executeView(null, 'bw_getdatasourceconfig', params);
	if(res==null || res.length==0) {
		// no config, just copy the file
		dataset.error.set( copyFile(sourcefilepath, destfilepath));	
		return;	
	}
	var record = res[0];
	...
	var res = connector.executeView(null, 'bw_getdatasourceallcolumns', params);
	if(res==null || res.length==0) {
		dataset.error.set( 'No CSV columns declared in the configuration') ;
		return;
	}
	...
	var res = connector.executeView(null, 'bw_getdatasourcemapping', params);
	if(res==null || res.length==0) {
		dataset.error.set( 'No column mapping declared in the configuration' );
		return;
	}
	...
	var fileparser = connector.getFileParser('CSV');	
	...
	try {
		...
		dataset.error.set( e.getMessage()) ;
		return;
	}
	finally {
...
function getActiveTimeslotUid(){
   var res=connector.executeView(null, "bwdsm_activetimeslot" )	;
   if (res != null && res.length>0){
...
function compute(/*String*/ expression, /*java.util.HashMap*/ records, /*String*/ outputType) {
	...
	var /*String*/ val = connector.executeMacro(expression, records, false, cache);

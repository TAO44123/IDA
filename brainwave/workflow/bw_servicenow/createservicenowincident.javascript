...
function createServiceNowIncident() {
    ...
    var res = createIncident(authenticationProperties, short_description, impact, urgency, strDate, caller, assignment_group, assigned_to, comments, cnames, cvalues);
	try {   
        if (res == null) {
	        ...
	        dataset.ticketid.set(res.result.sys_id);

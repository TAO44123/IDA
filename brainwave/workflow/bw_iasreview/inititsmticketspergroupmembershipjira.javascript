...
function createChangeInfos() {
	...
	var actionscsv = "status,identifier,login,repository,groupidentifier,groupname,grouprepository,comment,requestedon,requestedby\n";
	...
	for(var i=0;i<dataset.reviewstatus.length;i++) {
		...
		var groupidentifier = '"'+dataset.groupcode.get(i)+'"';
		var groupname = dataset.groupdisplayname.get(i);
		var grouprepository = dataset.grouprepositorydisplayname.get(i);
		...
		var str2 = status+","+identifier+","+login+","+repository+","+groupidentifier+","+groupname+","+grouprepository+","+comment.replace(",", "", "g")+","+on+","+by+"\n";		

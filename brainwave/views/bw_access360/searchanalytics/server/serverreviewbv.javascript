...
function advancedReviewerFinding(/*DataSet*/ record) {
  ...
  var repositoryowneruid = record.repositoryowneruid.get();
  var repositoryownerhrcode = record.repositoryownerhrcode.get();
  var repositoryownerfullname = record.repositoryownerfullname.get();
  var repositoryownermail = record.repositoryownermail.get();
  ...
  if ("leave".equals(record.reconnoownercode.get())) accounttype = "leave";
  else if (record.reconnoownercode.get() != null) accounttype = "technical";
  else if (record.identityuid.get() != null) accounttype = "user";
  else accounttype = "orphan";
...
function standardReviewerFinding(/*DataSet*/ record) {
  ...
  var /*String*/ repositoryowneruid = null;
  var /*String*/ repositoryownerhrcode = null;
  var /*String*/ repositoryownerfullname = null;
  var /*String*/ repositoryownermail = null;
  if (!record.isEmpty("repositoryownermail")) {
    repositoryowneruid = record.repositoryowneruid.get();
    repositoryownerhrcode = record.repositoryownerhrcode.get();
    repositoryownerfullname = record.repositoryownerfullname.get();
    repositoryownermail = record.repositoryownermail.get();
  }
  ...
  // STEP 2: Tech. accounts & orphan accounts are reviewed by repo owner
  if (!isUserAccount) {
    revieweruid = repositoryowneruid;
    reviewerhrcode = repositoryownerhrcode;
    reviewerfullname = repositoryownerfullname;
  ...
  // STEP 4: Option: All remaining accounts are reviewed by the repository owner
  if (
    dataset.forcerepositoryownerforallotheraccounts.get().equals("1") &&
    revieweruid == null
  ) {
    revieweruid = repositoryowneruid;
    reviewerhrcode = repositoryownerhrcode;
  ...
  // STEP 5: Option: All accounts are reviewed by the repo owner
  if (dataset.forcerepositoryownerasreviewer.get().equals("1")) {
    revieweruid = repositoryowneruid;
    reviewerhrcode = repositoryownerhrcode;
    reviewerfullname = repositoryownerfullname;
    reviewermail = repositoryownermail;
...
function initReviewColumns() {
  ...
  var /*Attribute<String>*/ attribute = record.add(
      "reviewstatus",
      "String",
      false
    );
...
function checkIdentityPosition() {
  ...
  if (
    identitypositionkey == null ||
    reviewidentitypositionkey == null ||
    !identitypositionkey.equals(reviewidentitypositionkey)
  ) {
